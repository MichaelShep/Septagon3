        ...
        if (movable.size() > 0) {
            for (int i = 1; i > -2; i--) {
                for (Tile tile : movable) {
                    System.out.println((int) this.getLocation().getMapX() + directionArray[Math.abs(direction + i) % 4][0] + " " + (int) this.getLocation().getMapY() + directionArray[Math.abs(direction + i) % 4][1]);
